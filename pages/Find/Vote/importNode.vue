<template>
	<view class="container b-b">
		...
		<!-- 开始导入 按钮 -->
		<btn-self text="确定导入" :state="allSelect" @btnClick="btnClick"></btn-self>
		<!-- 服务及隐私条款 -->
	...
	export default {
		data() {
			return {
				...
				dataList: ['私钥导入'],
				tabClick: 1,
				placeList: ['按顺序输入助记词,助记词之间请用一个空格分开。','请输入私钥'],
				psw: "",//密码
				psw_state: false,
				psw_password: true,
		...
		methods: {
			...
			async getMyCoin(options) {//账户信息
				...
				if(res.data.StatusCode == 200) {
					...
					if(value) {//存在
						...
						// 判断是不是节点
						let node = uni.getStorageSync('nodeList');//节点列表
						let nodeList = [];//我的节点列表
						let index1 = node.findIndex(x=> {
							return x.names == options
						})
						if(index1 !== -1) {//表示该钱包是节点
							let myNodeList = uni.getStorageSync('myNodeList');//我的节点列表
							if(myNodeList) {//我的节点列表存在
								let myIndex = myNodeList.findIndex(x=> {
									return x.names == options
								});
								if(myIndex == -1) {//不在我的节点列表中 我的节点列表追加
									nodeList.push(node[index1])
								}else {//在我的节点列表中
									nodeList = myNodeList
								}
							}else {//不存在
			...
			btnClick() {
				if(this.allSelect) {
					if(this.textarea.trim().length !== 51) {
						uni.showToast({
							icon: 'none',
							title: "私钥输入错误"
						});
